"""SSLCommerz payment service (production gateway)."""
...
from .mock_gateway import DuplicateTransactionError, PaymentError
from .models import (
...
class SSLCommerzConfig:
    ...
    def from_env(cls) -> "SSLCommerzConfig":
        ...
        api_url = os.getenv("SSLCOMMERZ_API", "https://sandbox-gw.sslcommerz.com/gwprocess/v4/api.php").strip()
    ...
    def _post_sandbox_session(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        with httpx.Client(timeout=timeout) as client:
            response = client.post(
                "https://sandbox-gw.sslcommerz.com/gwprocess/v4/api.php", data=payload
            )
            response.raise_for_status()
            data = response.json()
        if not isinstance(data, dict):
